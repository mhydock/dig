...
export class ToolBox {
  ...
  constructor(private game: Game, private tool: Tool) {
  ...
  private checkCost = (cost: number) => {
    ...
    if (cost > this.game.Money) this.buyBtn.disabled = true;
  };

  private checkMoney = (money: number) => {
    if (money < this.tool.BuyCost) this.buyBtn.disabled = true;
    else this.buyBtn.disabled = false;
  };
  ...
  private checkResearched = (level: number) => {
    ...
    else this.toolBox.style.display = "none";
  };
}
